...
export class UsersLogInPage implements OnInit, OnDestroy {
		...
		this.login.config = this.login.config || [
			{
				...
				Required: true,
				Options: {
					Type: this.configSvc.appConfig.accounts.isPhoneAllowed ? "text" : "email",
					Label: await this.configSvc.getResourceAsync(`users.login.login.controls.${this.configSvc.appConfig.accounts.isPhoneAllowed ? "Account" : "Email"}.label`),
					PlaceHolder: await this.configSvc.getResourceAsync(`users.login.login.controls.${this.configSvc.appConfig.accounts.isPhoneAllowed ? "Account" : "Email"}.placeholder`),
					MinLength: 1,
					MaxLength: 150,
					AutoFocus: true
				}
	...
	logIn() {
		if (this.appFormsSvc.validate(this.login.form)) {
			...
			this.appFormsSvc.showLoadingAsync(this.title).then(() => this.authSvc.logInAsync(
				...
				error => this.trackAsync(this.title).then(() => this.appFormsSvc.showErrorAsync(error, undefined, () => this.login.controls.find(ctrl => ctrl.Name === "Account").focus())))
			);
		}
	}
	...
	async openLoginOTPAsync(data: any) {
		...
		await this.trackAsync(this.title, this.configSvc.appConfig.URLs.users.otp, "Request", "Users:OTP");
	}
	...
	logInOTP() {
		if (this.appFormsSvc.validate(this.otp.form)) {
			this.appFormsSvc.showLoadingAsync(this.title).then(() => this.authSvc.logInOTPAsync(
				...
				() => this.trackAsync(this.title, this.configSvc.appConfig.URLs.users.otp, "Validate", "Users:OTP").then(() => this.appFormsSvc.hideLoadingAsync(() => this.close())),
				error => this.trackAsync(this.title, this.configSvc.appConfig.URLs.users.otp, "Validate", "Users:OTP").then(() => this.appFormsSvc.showErrorAsync(error, undefined, () => {
	...
	resetPassword() {
		...
		if (this.configSvc.appConfig.accounts.isPhoneAllowed && AppUtility.isPhone(account)) {
			...
			if (AppUtility.isEmpty(otp)) {
				...
				this.appFormsSvc.showLoadingAsync(this.title).then(() => this.authSvc.renewPasswordAsync(
					...
					error => this.appFormsSvc.showErrorAsync(error, undefined, () => {
						control.controlRef.deleteValue();
						control.focus(123);
